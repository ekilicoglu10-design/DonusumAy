// Müşteri temsilcisi paneli authentication JavaScript

// Demo kullanıcı bilgileri
const CUSTOMER_REP_USERS = [
    {
        ...
        level: 'senior',
        permissions: ['support', 'chat', 'tickets', 'reports']
    },
    {
        email: 'support@donusumay.com',
        password: 'support123',
        name: 'Ayşe Kaya',
        role: 'customer_representative',
        department: 'Teknik Destek',
        level: 'expert',
        permissions: ['support', 'chat', 'tickets', 'reports', 'technical']
    },
    {
        email: 'help@donusumay.com',
        password: 'help123',
        name: 'Mehmet Demir',
        role: 'customer_representative',
        department: 'Müşteri Hizmetleri',
        level: 'junior',
        permissions: ['support', 'chat', 'tickets']
    }
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (isLoggedIn === 'true' && userRole === 'customer_representative') {
        showNotification('Zaten oturum açmışsınız, dashboard\'a yönlendiriliyorsunuz...', 'success');
        setTimeout(() => {
            window.location.href = 'customer-representative-dashboard.html';
        }, 1000);
        return;
    }
});
...
// Müşteri temsilcisi giriş işlemi
async function handleCustomerRepLogin(event) {
    ...
    try {
        ...
        const user = CUSTOMER_REP_USERS.find(u => u.email === email && u.password === password);
        ...
        if (user) {
            ...
            localStorage.setItem('customer_rep_logged_in', 'true');
            localStorage.setItem('user_role', 'customer_representative');
            ...
            if (remember) {
                localStorage.setItem('customer_rep_remember', 'true');
            }
...
async function handleForgotPassword(event) {
    ...
    const user = CUSTOMER_REP_USERS.find(u => u.email === email);
...
function fillDemoCredentials() {
    document.getElementById('email').value = 'temsilci@demo.com';
